...
void visualizer::runVisualizerOri()
{
    ...
    {
        if (checkScale_ == false)
        {
            scaleX_ = vecVertexInfo_[i].x;
            scaleY_ = vecVertexInfo_[i].y;
            scaleZ_ = vecVertexInfo_[i].z;
        }
        
        marker.id = id;
        marker.pose.position.x = vecVertexInfo_[i].x - scaleX_;
        marker.pose.position.y = vecVertexInfo_[i].y - scaleY_;
        marker.pose.position.z = vecVertexInfo_[i].z - scaleZ_;
    ...
    for (int i = 0; i < vecEdgeInfo_.size(); i++)
    {
        ...
        p.x = vecVertexInfo_[vecEdgeInfo_[i].id_1].x - scaleX_;
        p.y = vecVertexInfo_[vecEdgeInfo_[i].id_1].y - scaleY_;
        p.z = vecVertexInfo_[vecEdgeInfo_[i].id_1].z - scaleZ_;
        edge.points.push_back(p);

        p.x = vecVertexInfo_[vecEdgeInfo_[i].id_2].x - scaleX_;
        p.y = vecVertexInfo_[vecEdgeInfo_[i].id_2].y - scaleY_;
        p.z = vecVertexInfo_[vecEdgeInfo_[i].id_2].z - scaleZ_;
        edge.points.push_back(p);
        edge.id = id;
...
void visualizer::runVisualizerChg(gtsam::NonlinearFactorGraph::shared_ptr &graph_, gtsam::Values &result)
{
    ...
    for (const gtsam::Values::KeyValuePair key_value : result)
    {
        ...
        marker.id = id;
        marker.pose.position.x = t.x() - scaleX_;
        marker.pose.position.y = t.y() - scaleY_;
        marker.pose.position.z = t.z() - scaleZ_;
        markerArray.markers.push_back(visualization_msgs::Marker(marker));
        id++;
